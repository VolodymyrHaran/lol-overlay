package consumers

import (
	"context"
	"encoding/json"
	"fmt"
	"log"

	"github.com/nats-io/nats.go"

	"lol-timer/internal/messaging"
	"lol-timer/internal/models"
	"lol-timer/internal/services"
)

type RoomBroadcaster interface {
	BroadcastCurrentRoom(roomID string)
	BroadcastRoomUpdate(room *models.Room)
}

type RoomConsumer struct {
	nats        *messaging.Client
	broadcaster RoomBroadcaster
	roomService *services.RoomService
}

func NewRoomConsumer(
	natsClient *messaging.Client,
	broadcaster RoomBroadcaster,
	roomService *services.RoomService,
) *RoomConsumer {
	return &RoomConsumer{
		nats:        natsClient,
		broadcaster: broadcaster,
		roomService: roomService,
	}
}

func (c *RoomConsumer) Start() error {
	_, err := c.nats.Subscribe(
		messaging.SubjectCurrentRoomChanged,
		func(msg *nats.Msg) {
			c.handleCurrentRoomChanged(
				msg.Data,
			)
		},
	)
	if err != nil {
		return fmt.Errorf(
			"subscribe current room changed: %w",
			err,
		)
	}

	_, err = c.nats.Subscribe(
		messaging.SubjectRoomUpdated,
		func(msg *nats.Msg) {
			c.handleRoomUpdated(
				context.Background(),
				msg.Data,
			)
		},
	)

	if err != nil {
		return fmt.Errorf(
			"subscribe room updated: %w",
			err,
		)
	}

	if err := c.nats.Flush(); err != nil {
		return fmt.Errorf(
			"confirm room subscriptions: %w",
			err,
		)
	}

	return nil
}

func (c *RoomConsumer) handleCurrentRoomChanged(
	data []byte,
) {
	var event messaging.CurrentRoomChangedEvent

	if err := json.Unmarshal(
		data,
		&event,
	); err != nil {
		log.Printf(
			"decode current room changed event: %v",
			err,
		)
		return
	}

	c.broadcaster.BroadcastCurrentRoom(
		event.RoomID,
	)
}

func (c *RoomConsumer) handleRoomUpdated(
	ctx context.Context,
	data []byte,
) {
	var event messaging.RoomUpdatedEvent

	if err := json.Unmarshal(
		data,
		&event,
	); err != nil {
		log.Printf(
			"decode room updated event: %v",
			err,
		)
		return
	}

	if event.RoomID == "" {
		log.Printf(
			"room updated event has empty room ID",
		)
		return
	}

	room, err := c.roomService.GetRoomSnapshot(
		ctx,
		event.RoomID,
	)
	if err != nil {
		log.Printf(
			"load updated room: room=%q error=%v",
			event.RoomID,
			err,
		)
		return
	}

	if room == nil {
		log.Printf(
			"updated room not found: room=%q",
			event.RoomID,
		)
		return
	}

	log.Printf(
		"room updated event consumed: room=%q",
		event.RoomID,
	)

	c.broadcaster.BroadcastRoomUpdate(room)
}
